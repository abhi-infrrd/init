...
def init_hybrid():
    ...
    try:
        #if pickle exists
        logger.info('Reading pickle')
        (model, d_ui, courses_with_metadata_index, courses_with_metadata, products_with_metadata_index, products_with_metadata, dictionary_courses_with_metadata, dictionary_products_with_metadata) = load_pickle_item_id()
    except:
        #if pickle doesn't exist
        logger.info('Training the model, pickle doesn\'t exist')
